...
#[tokio::test]
async fn simple_closure() { expect_script("a=1;f=fn(){=a};=f()", 1).await; }

#[tokio::test]
async fn closure_inline() { expect_script("a=1;=fn(){=a}()", 1).await; }

#[tokio::test]
async fn parameter_closed() { expect_script("f=fn(a){=fn(){=a}};fx=f(1);=fx()", 1).await; }

#[tokio::test]
async fn partial_sum() { expect_script("f=fn(a){=fn(b){=a+b}};fx=f(1);=fx(2)", 3).await; }

#[tokio::test]
async fn closure_fake_recurse() { expect_script("f=fn(f,a){=if a < 1 {=a} else {=a + f(f,a-1)}}; =f(f,3)", 6).await; }
