...
use util::{expect_bool, expect_f64, expect_i64, expect_str};

#[tokio::test]
async fn simple_int() { expect_i64("=1", 1).await; }
...
#[tokio::test]
async fn expr_math() { expect_i64("= 1 + 1", 2).await; }

#[tokio::test]
async fn expr_subt() { expect_i64("= 2 - 1", 1).await; }

#[tokio::test]
async fn expr_math_prec() { expect_i64("= 1 + 1 * 2", 3).await; }

#[tokio::test]
async fn expr_math_complex() { expect_i64("= 2 * 1 + 1 * 2", 4).await; }
...
#[tokio::test]
async fn expr_grouping() { expect_i64("= (1 + 1) * 2", 4).await; }
